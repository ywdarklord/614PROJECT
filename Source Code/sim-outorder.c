...
/* tournament predictor config (<sel_size> <global_regsize> <local_htb_size> <local_hrsize>)*/
static int tournament_nelt=4;
static int tournament_config[4]={4906, 12,1024,1024};
...
"  Predictor `comb' combines a bimodal and a 2-level predictor.\n"
  ...
  opt_reg_int_list(odb, "-bpred:tournament", "tournament predictor config (<sel_size> <global_regsize> <local_htb_size> <local_hrsize>)", tournament_config, tournament_nelt, &tournament_nelt, tournament_config, TRUE, NULL, FALSE);
  ...
  if (!mystricmp(pred_type, "perfect"))
	...
	fatal("bad btb config (<num_sets> <associativity>)");
      ...
      if (tournament_nelt!=4) {
          fatal("bad tournament predictor config (<sel_size> <global_regsize> <local_htb_size> <local_hrsize>)");
      }
      
      pred=bpred_create(BPredTournament,
                        /*global_reg_width*/tournament_config[1],
                        /*local_htb_size*/tournament_config[2],
                        /*local_hrsize*/tournament_config[3],
                        /*sel_size*/tournament_config[0],
                        /*local history reg size*/10,
                        0,
                        /*btb sets*/btb_config[0],
                        /*btb assoc*/btb_config[1],
                        /*ret-addr*/ras_size);
